#include<bits/stdc++.h>
using namespace std;
#define ll long long int
#define ull unsigned long long int
#define has(s, key) (s.find(key) != s.end())
#define fast ios_base::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL)
#define vi vector<int>
#define vl vector<ll>
#define v(a) vector<a>
#define si set<int>
#define usi unordered_set<int>
#define pii pair<int, int>
#define vpp vector<pair<int, pii>>
#define rset(dp) memset(dp,-1,sizeof(dp))
#define pq_max priority_queue<int>
#define pq_min priority_queue<int, vi, greater<int>>
#define print(a) for(auto x : a) cout<<x<<" "; cout<<endl
#define printp(a) for(auto x : a) cout<<x.F<<" "<<x.S<<endl
#define printa(a,x,y) for(int i = x; i < y ; i++) cout<<a[i]<<" "; cout<<endl
#define F first
#define S second
#define pb push_back
#define mii map<int,int>
#define umii unordered_map<int,int>
#define umap unordered_map
#define all(p) b.begin(), p.end()
#define ina(a, n) for(int i=0;i<n;i++) cin>>a[i]
#define inp(a, n) for(int i=0;i<n;i++) cin>>a[i].F>>a[i].S;
#define inq(q,n,t) for(int i=0;i<n;i++) for(int j=0;j<t;j++) cin>>q[i][j]

int sortKp(vector<int>& arr, int n, int k) {
   	vector<pair<int,int>> repos;
	vector<int> pos(n + 1);
   	for(int i = 1; i <= n; i++)
         pos[arr[i]] = i;
   	bool can = false;
   	for(int i = 1; i <= n; i++){
    	if(abs(pos[i] - i) % k != 0){
        	can = true;
        	break;
    	}
   	}
   	if(!can)
		return 0;
   	for(int i = 1; i <= n;i++)
    	if((pos[i] - i) % k != 0)
        	repos.push_back(make_pair(i,abs(pos[i]-i)%k));
    if(repos.size() > 2)
        return -1;
    int index = repos[0].first;
    int index2 = repos[1].first;
    pos[arr[index]] = index2;
    pos[arr[index2]] = index;
    swap(arr[index], arr[index2]);
    for(int i = 1;i <= n; i++) {
        if((pos[arr[i]] - i)% k != 0)
            return -1;
    }
    return 1;
}

int main() {
	fast;
	clock_t z = clock();
	int t, n, k;
	cin>>t;
	while(t--) {
		cin>>n>>k;
		vector<int> p(n + 1);
		for(int i = 1; i <= n; i++)
			cin>>p[i];
		cout<<sortKp(p, n, k)<<endl;
	}
	cerr << "Run Time : "<<((double)(clock() - z)/CLOCKS_PER_SEC);
	return 0;
}